...
export default function EventsDashboard() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!token) {
        alert("Authentication Required: We couldn't find your login session. Please log in again.");
        return;
      }
    ...
    <div className="flex min-h-screen bg-slate-950 text-white font-sans selection:bg-emerald-500/30">
      ...
      <aside className="w-64 border-r border-white/10 bg-slate-900/50 backdrop-blur-xl p-6 hidden md:block">
        <div className="mb-10 flex items-center gap-3">
          <div className="bg-emerald-500 p-2 rounded-lg text-slate-950 font-bold shadow-lg shadow-emerald-500/20">UC</div>
        ...
        <nav className="space-y-2">
          <button 
            ...
            className={`w-full text-left px-4 py-3 rounded-xl transition-all duration-200 ${view === 'overview' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 shadow-inner' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
          >
            📊 Event Dashboard
          </button>
          <button 
            onClick={() => setView('register')}
            className={`w-full text-left px-4 py-3 rounded-xl transition-all duration-200 ${view === 'register' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 shadow-inner' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
          >
            ➕ Register New Event
          </button>
        </nav>
      ...
      <main className="flex-1 p-10 overflow-y-auto">
        <header className="mb-10 flex justify-between items-end border-b border-white/5 pb-8">
          <div>
            ...
            <p className="text-slate-400 mt-2 font-medium">Manage your community sustainability programs.</p>
          </div>
          {view === 'overview' && (
             <div className="bg-slate-900 border border-white/10 px-6 py-3 rounded-2xl text-sm font-semibold shadow-xl">
                Active Programs: <span className="text-emerald-400 ml-2">{events.length}</span>
             </div>
          )}
        </header>
        ...
        {view === 'overview' ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {events.length > 0 ? events.map((event) => (
              <div key={event._id} className="bg-slate-900/80 border border-white/10 p-6 rounded-3xl hover:border-emerald-500/50 transition-all duration-300 group shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 left-0 w-1 h-full bg-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                <div className="flex justify-between items-start mb-4">
                  <span className="bg-emerald-500/10 text-emerald-400 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest border border-emerald-500/20">Live</span>
                  <button 
                ...
                <p className="text-slate-400 text-sm line-clamp-3 mb-6 leading-relaxed">{event.description}</p>
                <div className="text-[11px] text-slate-500 space-y-3 pt-5 border-t border-white/5 font-medium">
                  <p className="flex items-center gap-2">
                    <span className="opacity-50 text-base">📍</span> {event.location}
                  </p>
                  <p className="flex items-center gap-2">
                    <span className="opacity-50 text-base">📅</span> {new Date(event.date).toLocaleDateString(undefined, { dateStyle: 'long' })}
                  </p>
                </div>
              </div>
            )) : (
              <div className="col-span-full py-20 text-center border-2 border-dashed border-white/5 rounded-3xl">
                <p className="text-slate-500 italic text-lg">No community events listed yet.</p>
                <button onClick={() => setView('register')} className="mt-4 text-emerald-400 hover:underline">Click here to add the first one</button>
              </div>
            )}
          </div>
        ) : (
          ...
          <div className="max-w-3xl mx-auto bg-slate-900 border border-white/10 rounded-[2rem] p-10 shadow-2xl relative overflow-hidden">
             <div className="absolute top-0 right-0 p-8 opacity-5 text-6xl font-black">FORM</div>
             <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   ...
                   <div className="flex flex-col gap-3">
                      <label className="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Event Date</label>
                      <input 
                        type="date" 
                   ...
                   <div className="flex flex-col gap-3">
                      <label className="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Host Organization</label>
                      <input 
                        className="bg-slate-950 p-4 rounded-2xl border border-white/10 outline-none focus:border-emerald-500 transition-all text-white placeholder:text-slate-700" 
                ...
                <div className="flex flex-col gap-3">
                   <label className="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Program Details</label>
                   <textarea 
                     rows="5" 
